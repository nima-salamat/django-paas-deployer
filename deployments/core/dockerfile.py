...
def _php_entrypoint_script(
    *,
    ...
    lines = [
        ...
        '        echo "[entrypoint] SQLite database file created at $DB_FILE"',
        '      else',
        '        echo "[entrypoint] WARNING: SQLite database path is not writable: $DB_FILE" >&2',
        '        echo "[entrypoint] Mount a writable volume for SQLite or use an external database." >&2',
        '      fi',
        "    fi",
        '    chmod ug+rw "$DB_FILE" 2>/dev/null || true',
        '    export DB_DATABASE="$DB_FILE"',
        "  fi",
        '  # Runtime environment is injected by the orchestrator/container. Do not create or mutate .env.',
        '  if [ -n "${APP_KEY:-}" ] || [ -n "${DB_CONNECTION:-}" ] || [ -n "${DB_DATABASE:-}" ]; then',
        '    echo "[entrypoint] Using injected container environment; .env creation/mutation disabled."',
        '  elif [ -f .env ]; then',
        '    echo "[entrypoint] Using existing .env because no corresponding injected runtime variables were provided."',
        '  else',
        '    echo "[entrypoint] No .env and no injected APP_KEY/DB_* variables; continuing without creating project files."',
        '  fi',
        '  echo "[entrypoint] Running package:discover ..."',
        '  php artisan package:discover --ansi 2>/dev/null || true',
        '  if [ -z "${APP_KEY:-}" ]; then',
        '    echo "[entrypoint] APP_KEY is not injected; skipping key:generate because runtime filesystem is immutable."',
        '  fi',

...
def test_base_image_build_is_dedicated_task_and_routed():
    tasks = (ROOT / "deployments/celery/tasks.py").read_text(encoding="utf-8")
    settings = (ROOT / "config/settings.py").read_text(encoding="utf-8")
    assert "build_registered_base_image(" in tasks
    assert "task_id=str(self.request.id)" in tasks
    assert "force_rebuild=bool(force_rebuild)" in tasks
...
def test_force_cancel_releases_base_leases_and_preserves_running_service():
    runtime = (ROOT / "services/api/runtime.py").read_text(encoding="utf-8")
...
def test_base_runtime_image_has_build_task_ownership_fields():
    models = (ROOT / "deploy/models.py").read_text(encoding="utf-8")
    migration_task = (ROOT / "deploy/migrations/0012_base_runtime_build_task.py").read_text(encoding="utf-8")
    migration_owner = (ROOT / "deploy/migrations/0013_base_runtime_build_owner.py").read_text(encoding="utf-8")
